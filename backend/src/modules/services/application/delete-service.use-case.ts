 ...
 *
 * Application use case for deleting a service.
 * This use case orchestrates domain entities and domain services to delete a service.
 *
 * Responsibilities:
...
export class DeleteServiceUseCase {
   ...
   *
   * @param input - Input data for deleting service
  ...
  async execute(input: DeleteServiceInput): Promise<DeleteServiceResult> {
    try {
      ...
      if (!hasManagerOrOwnerRole) {
        throw new ForbiddenError('Only Manager or Owner role can delete services');
      }
      ...
      await this.createAuditLog(existingService, input.performedBy);
  ...
  /**
   * Creates audit log entry for service deletion
   */
   ...
   */
  private handleError(error: unknown): DeleteServiceResult {
