...
export class AppointmentPricingDomainService {
  ...
  calculatePricingBreakdown(
    ...
    for (const line of serviceLines) {
      ...
      if (line.appointmentId !== appointment.id) {
        throw new Error(
          `Service line ${line.id} does not belong to appointment ${appointment.id}`
        );
      }
      ...
      if (!service) {
        throw new Error(
          `Service ${line.serviceId} referenced by line ${line.id} not found`
        );
      }
  ...
  private validateAppointmentHasLines(
    ...
    if (!serviceLines || serviceLines.length === 0) {
      throw new Error(
        `Appointment ${appointment.id} must have at least one service line`
      );
    }
  }
