...
export class PhoneNumber {
  ...
  private validatePhone(phone: string): void {
    ...
    if (digits.length < 7) {
      throw new Error(`Phone number must contain at least 7 digits. Found: ${digits.length}`);
    }

    // Validate maximum digit count (15 digits max for international format including country code)
    if (digits.length > 15) {
      throw new Error(`Phone number cannot contain more than 15 digits. Found: ${digits.length}`);
    }
