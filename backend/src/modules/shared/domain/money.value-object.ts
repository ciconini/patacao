...
export class Money {
  ...
  equals(other: Money | null | undefined): boolean {
    ...
    return (
      Math.abs(this._amount - other._amount) < 0.005 && // 0.005 = half of 0.01
  ...
  static sum(monies: Money[]): Money {
    ...
    for (const money of monies) {
      if (money._currency !== currency) {
        throw new Error(
          `Cannot sum Money instances with different currencies: ${currency} and ${money._currency}`,
        );
      }
      total += money._amount;
    }
  ...
  static max(monies: Money[]): Money {
    ...
    for (const money of monies) {
      if (money._currency !== currency) {
        throw new Error(
          `Cannot compare Money instances with different currencies: ${currency} and ${money._currency}`,
        );
      }
  ...
  private ensureSameCurrency(other: Money): void {
    if (this._currency !== other._currency) {
      throw new Error(
        `Cannot perform operation on Money with different currencies: ${this._currency} and ${other._currency}`,
      );
    }
  }
}
